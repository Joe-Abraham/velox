  ...
  for (const auto& childSpec : childrenSpecs) {
    ...
    if (auto iter = splitInfoColumns.find(fieldName);
      ...
      if (outputTypeIdx.has_value() && fileTypeIdx.has_value()) {
        ...
        //    position.
        // 5. Equality-delete partition columns not in the user's projection:
        ...
        if (fieldName ==
          ...
          // Constant already set (case 5, or set on a previous prepareSplit
          // call for the same scanSpec). Nothing to do.
          continue;
        } else if (
            auto it = fileSplit_->partitionKeys.find(fieldName);
            it != fileSplit_->partitionKeys.end()) {
          setPartitionValue(childSpec.get(), fieldName, it->second);
        } else {
          VELOX_CHECK_NOT_NULL(
              tableSchema,
              "Table schema is required to resolve type for column not in file: {}",
              fieldName);
          childSpec->setConstantValue(
              BaseVector::createNullConstant(
