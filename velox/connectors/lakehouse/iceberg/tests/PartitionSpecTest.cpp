...
TEST_F(PartitionSpecTest, fieldSerializeRoundTrip) {
  ...
  IcebergPartitionSpec::Field fieldWithParam(
      "bucket_column", TransformType::kBucket, 10);
  ...
  auto field =
      std::static_pointer_cast<const IcebergPartitionSpec::Field>(deserialized);
  ...
  IcebergPartitionSpec::Field fieldWithoutParam(
      "identity_column", TransformType::kIdentity, std::nullopt);
  ...
  auto field2 = std::static_pointer_cast<const IcebergPartitionSpec::Field>(
      deserialized2);
...
TEST_F(PartitionSpecTest, partitionSpecSerializeRoundTrip) {
  ...
  auto deserializedSpec =
      std::static_pointer_cast<const IcebergPartitionSpec>(deserialized);
  EXPECT_EQ(deserializedSpec->specId, 42);
  EXPECT_EQ(deserializedSpec->fields.size(), 4);

  // Verify deserialized fields
  const auto& deserializedFields = deserializedSpec->fields;
...
TEST_F(PartitionSpecTest, serializationErrors) {
  ...
  EXPECT_THROW(
      IcebergPartitionSpec::Field::create(invalidField, pool()),
      VeloxException);
  ...
  EXPECT_THROW(
      IcebergPartitionSpec::create(invalidSpec, pool()), VeloxException);
}
