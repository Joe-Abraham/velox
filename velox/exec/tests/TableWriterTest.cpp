...
// Covers the NOT NULL constraint carried by
// InsertTableHandle::notNullColumns().
class TableWriterNotNullTest : public HiveConnectorTestBase {
 protected:
  // Plans a write of 'input' into a fresh temporary directory, enforcing NOT
  // NULL on 'notNullColumns'. 'targetColumns', when not null, selects and
  // reorders the written columns.
  ...
  // Runs 'plan' and returns the number of rows written.
  ...
  // Output directories of the plans built so far, kept alive for the duration
  // of the test.
...
TEST_F(TableWriterNotNullTest, enforcement) {
  ...
  // The same nulls presented flat and behind dictionary and constant wrapping.
  ...
  for (const auto& nullColumn : nullColumns) {
    ...
    // c0 has no nulls, so constraining it leaves the nulls in c1 alone.
...
// past the RowVector's size are not part of the batch.
...
// The target table's column names may differ from the input's, in which case
// the constraint must be expressed in the table's names. PlanBuilder always
// writes under the input names, so build the node directly.
...
// The DecodedVector and SelectivityVector reused across batches must neither
// break on a change of size or encoding nor mask a null in a later batch.
